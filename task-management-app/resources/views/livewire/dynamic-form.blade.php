    ...
    <form action="{{ route('activity.goal.store') }}" method="POST">
        ...
        @for ($i = 1; $i <= $inputCount; $i++)
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="goalName-{{ $i }}" id="selectGoals">
            ...
            <div class="form-group">
                <label for="titleInput">Objectives</label>

                <select class="form-control" name="GoalObjective-{{ $i }}" id="">

                    {{-- @foreach ($allObjectives as $objective)
                    <option value="{{ $objective->id }}" wire:model='objective_id'>{{ $objective->name }}</option>
                    @endforeach --}}
